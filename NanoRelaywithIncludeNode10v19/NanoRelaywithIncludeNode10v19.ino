...
***********************************************************************************************************************/
const char Version[] = "NaNoRelayNode10V19";
uint8_t G_RFTXPin = 2; // tx = pin 2 RF433
uint8_t G_RFRXPin = 10; // rx  = pin 10 RF433
uint8_t my_blueLed = 7; // receiving = pin 4 //visual control pins
uint8_t my_greenLed = 8; // transmitting = pin 5 //visual control pins
uint8_t my_redLed = 6;  // error somewhere = pin 7 //visual control pins //not that simple on ATtiny 
...
int relayPinsNumber[5]={9,9,13,99,99}; // 9 and 9 are for IR pin, 13 for the inboard led, 99 means unused  
...
unsigned long clearCMDHistoryAfterMicros = 30000000; //we keep an history of command to avoid repeating the same action on relayPinsNumber[x] (as by default sender will send the same command x times over RF433), this delay will clear the table entries after a while
char histCommandSummary[6][5] = {"0000","1111","2222","3333","4444","5555"}; // Table of passed commands
...
#ifdef DHTPRESENT
#define DHTPIN 3     // what pin we're connecting to DHT22 sensor (has to be = pin number 0 in uint8_t pinsToTransmit[5]), pseudo pin allowing to send temperature from DHT22
#define DHTPIN2 4    // Pseudo pin number to allow sending humidity from DHT22 (has to be = pin number 1 in uint8_t pinsToTransmit[5])
...
#define NODE_NUMBER 10 // 0 to 254, unique for each emmitor node
uint8_t nodeNumber = 10; 
char G_myVerb[4] = "R10" ; // that's the verb used when we are transmitting our own sensors. 
DiOremote myRemote = DiOremote(G_RFTXPin); // instanciate a DiOremote object on RF433 emmitor pin
// include the generic code defined in C:\Users\pascal olin DA\Google Drive\Private\perso\Everycircuit\nano
#include <GenericHomerRelay.v19.h>
